...
IMPLEMENT_DYNAMIC(CBubleWnd, CWnd)
...
/************************************************************************/
/* function:创建一个子窗口，输入参数为父窗口指针                        */
/************************************************************************/
int CBubleWnd::Create(CWnd *pWndParent)
{
	m_pWndParent=pWndParent;	//保存父窗口指针
...
/* function:设置字体样式                                                */
/************************************************************************/
void CBubleWnd::SetTextFont(LPCTSTR szFont,int nSize,int nNormalStyle,int nSelectedStyle)
{
	LOGFONT lf;
...
/* function:设置字体颜色                                               */
/************************************************************************/
void CBubleWnd::SetTextColor(COLORREF crNormalTextColor,COLORREF crSelectedTextColor)
{
...
/* function:设置字体出现位置                                             */
/************************************************************************/
void CBubleWnd::SetTextRect(RECT rcText)
{
	m_rcText=rcText;
}
/************************************************************************/
/* function:设置冒泡窗口背景                                                */
/************************************************************************/
BOOL CBubleWnd::SetSkin(UINT nBitmapID,short red,short green,short blue)
{
	BITMAP bm;
...
/* function:显示冒泡窗口                                               */
/************************************************************************/
void CBubleWnd::Show(LPCTSTR szCaption,DWORD dwTimeToShow,DWORD dwTimeToLive,DWORD dwTimeToHide,int nIncrement)
{
...
HRGN CBubleWnd::GenerateRegion(HBITMAP hBitmap, BYTE red, BYTE green, BYTE blue)
{
...
BYTE* CBubleWnd::Get24BitPixels(HBITMAP pBitmap, WORD *pwWidth, WORD *pwHeight)
{
	BITMAP bmpBmp;
...
BEGIN_MESSAGE_MAP(CBubleWnd, CWnd)
	ON_WM_CREATE()
	ON_WM_MOUSEMOVE()
	ON_WM_DESTROY()
	ON_WM_ERASEBKGND()
...
// CBubleWnd 消息处理程序
int CBubleWnd::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
...
void CBubleWnd::OnMouseMove(UINT nFlags, CPoint point)
{
...
void CBubleWnd::OnLButtonUp(UINT nFlags, CPoint point)
{
	m_pWndParent->PostMessage(WM_TASKBARNOTIFIERCLICKED,0,0);
	CWnd::OnLButtonUp(nFlags, point);
}

LRESULT CBubleWnd::OnMouseHover(WPARAM w, LPARAM l)
{
...
LRESULT CBubleWnd::OnMouseLeave(WPARAM w, LPARAM l)
{
...
BOOL CBubleWnd::OnEraseBkgnd(CDC* pDC)
{
	CDC memDC;
	CBitmap *pOldBitmap;
	BITMAP bm;
...
void CBubleWnd::OnPaint()
{
	...
	//memcpy(szBuffer, m_strCaption.GetBuffer(), sizeof(szBuffer)*m_strCaption.GetLength());
...
BOOL CBubleWnd::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
...
void CBubleWnd::OnTimer(UINT nIDEvent)
{
	switch (nIDEvent)
	{
	case IDT_APPEARING:
